...
const PLATFORMS = [
  ...
  },
  {
    ...
    gen: 4,
    rarity: "uncommon",
    blurb: "Sega's colour answer to the Game Boy: essentially a Master System in your palms — the same Z80, the same VDP lineage — behind a backlit colour screen Nintendo's green monochrome couldn't touch. The price was paid at the battery door: six AA cells drained in a few hours. Bright, thirsty, and blessed with the Master System's well-charted homebrew roads.",
  },
  {
    id: "gamecom",
    name: "Tiger Game.com",
...
const PROJECTS = [
  ...
  /* ---- Sega Game Gear ---- */
  {
    ...
    status: "ongoing",
    progress: 65,
    summary: "The Game Boy's defining RPG carried onto Sega's colour handheld — not by rewriting it, but by transpiling the open-source pokered disassembly through a from-scratch Python transpiler (a full RGBDS front-end) into WLA-DX Z80. Around it sits a hand-written HAL bridging every hardware gulf between the two machines: the VDP driven through ports instead of memory-mapped VRAM, the Sega mapper standing in for the Game Boy's MBC, shadow-OAM rebuilt into the Sega SAT, HRAM relocated, and the subtle traps handled one by one — a reti that must not re-enable interrupts, swap's exact flags, SRAM windows that shift. And then the wall fell — and kept falling. The white screen before the title turned out to be three distinct bugs stacked atop one another; past them the title screen appears, and the whole opening flow runs: main menu, New Game, Oak's speech, naming, and out into the overworld, looping without a crash. The map renders, the player walks, Oak's dialogue boxes open. The Game Gear also found its voice: a hand-written PSG SN76489 driver now carries the sound effects, the monster cries and the dialog blips one-to-one with a software envelope, while the music note-stream — read cross-bank through a bank-0 helper — plays the title theme aloud. Getting here meant a long hunt through the emulator with live watchpoints (Emulicious over DAP): a WRAM wipe that erased the homecall trampoline, the stack living inside a map buffer, a real NMOS ld a,i interrupt quirk, a non-atomic VDP latch, and — the deepest — making the compressed homecall reentrant, after which the starter choice and the rival battle finally resolve and the game plays on. And then it became a game. With the overworld's map-load tilemap, its scroll seam, an emulated window layer and on-the-fly sprite mirroring all put right, the whole opening loop now runs playable end to end: random battles fought with correct data, levelling, the walk up Route 1 to Viridian, Oak's parcel bought at the mart, and the Pokédex received and working from the menu — a walking demo turned, bar a few cosmetic glitches, into a real game. Still ahead: the last graphical polish (a menu sprite-flip, a diaeresis haunting one lab dialogue, a music hang in places), the Game Gear's own hardware limits (eight sprites per scanline against the Game Boy's ten) deferred to a final pass, and validation across the whole long game. A technical study built on the open pokered disassembly: documentation only, nothing distributed, bring your own game.",
    milestones: [
      ...
      { label: "All 50 banks transpile, assemble and link clean into a 1 MB .gg; boots to the Game Freak intro", done: true },
      { label: "Milestone 1 — the title screen: the white-screen wall (a stack of three bugs) broken", done: true },
      { label: "Milestone 2 — full flow: New Game → Oak → naming → overworld, looping crash-free", done: true },
      { label: "Milestone 3 — overworld alive: map rendering, movement, dialogue boxes", done: true },
      { label: "Audio: PSG SN76489 driver singing — SFX, cries and cross-bank music, software envelope", done: true },
      { label: "Reentrant homecalls: the starter choice and the rival battle resolve at last", done: true },
      { label: "Overworld correctness pass: map-load tilemap, delta-scroll seam, an emulated window layer, on-the-fly sprite flip", done: true },
      { label: "Milestone 4 — a complete, playable game loop: random battles, levelling, shops, Oak's parcel, the Pokédex", done: true },
      { label: "Graphical polish (menu flips, a lab-dialogue glyph, a music hang), GG hardware limits, and whole-game validation — the long climb", done: false },
    ],
...
const I18N = {
  it: {
    ...
    platforms: {
      ...
      gamegear: { blurb: "La risposta a colori di Sega al Game Boy: in pratica un Master System nel palmo — lo stesso Z80, la stessa stirpe di VDP — dietro uno schermo a colori retroilluminato che il monocromatico verde di Nintendo non poteva eguagliare. Il prezzo si pagava allo sportello delle batterie: sei stilo AA prosciugate in poche ore. Luminosa, assetata, e benedetta dalle strade homebrew ben cartografate del Master System." },
    ...
    projects: {
      ...
      "gg-pokered": {
        ...
        summary: "L'RPG che ha definito il Game Boy portato sulla portatile a colori di Sega — non riscrivendolo, ma transpilando il disassemblaggio open source pokered attraverso un transpiler Python scritto da zero (un front-end RGBDS completo) verso il WLA-DX Z80. Attorno c'è un HAL scritto a mano che colma ogni abisso hardware fra le due macchine: il VDP pilotato tramite porte invece della VRAM mappata in memoria, il mapper Sega al posto dell'MBC del Game Boy, lo shadow-OAM ricostruito nel SAT di Sega, l'HRAM rilocata, e le trappole sottili gestite una a una — un reti che non deve riabilitare gli interrupt, i flag esatti di swap, le finestre SRAM che si spostano. E poi il muro è caduto — e ha continuato a cadere. Lo schermo bianco prima del titolo si è rivelato tre bug distinti impilati l'uno sull'altro; superati quelli appare la title screen, e gira tutto il flusso d'apertura: menu principale, New Game, il discorso di Oak, il naming, e fuori nell'overworld, in loop senza un crash. La mappa si disegna, il personaggio cammina, i box di dialogo di Oak si aprono. E il Game Gear ha trovato la voce: un driver PSG SN76489 scritto a mano porta ora gli effetti sonori, i versi dei mostri e i blip dei dialoghi uno-a-uno con un envelope software, mentre il note-stream della musica — letto cross-bank tramite un helper nel banco 0 — suona la sigla del titolo a voce alta. Arrivarci ha voluto dire una lunga caccia dentro l'emulatore con watchpoint dal vivo (Emulicious via DAP): una wipe della WRAM che cancellava il trampolino degli homecall, lo stack che viveva dentro un buffer di mappa, un vero quirk NMOS di ld a,i sugli interrupt, un latch VDP non atomico e — il più profondo — rendere rientrante l'homecall compresso, dopo di che la scelta dello starter e la battaglia col rivale si risolvono finalmente e il gioco prosegue. E poi è diventato un gioco. Con la tilemap al caricamento mappa, il seam dello scroll, un layer window emulato e il mirroring degli sprite al volo tutti sistemati, l'intero loop d'apertura ora è giocabile dall'inizio alla fine: battaglie casuali con dati corretti, livellaggio, la salita di Route 1 verso Viridian, il parcel di Oak comprato al mart, e il Pokédex ricevuto e funzionante dal menu — una demo che camminava diventata, a parte qualche glitch cosmetico, un gioco vero. Ancora davanti: le ultime rifiniture grafiche (un flip di sprite nei menu, una dieresi che infesta un dialogo del laboratorio, un mini-hang della musica in certi punti), i limiti hardware propri del Game Gear (otto sprite per scanline contro i dieci del Game Boy) rimandati a una passata finale, e la validazione sull'intero, lungo gioco. Uno studio tecnico costruito sul disassemblaggio aperto pokered: solo documentazione, nulla di distribuito, porta il tuo gioco.",
        milestones: [
          ...
          "Tutti i 50 banchi transpilano, assemblano e linkano puliti in un .gg da 1 MB; boota all'intro Game Freak",
          "Milestone 1 — la title screen: il muro dello schermo bianco (tre bug impilati) abbattuto",
          "Milestone 2 — flusso completo: New Game → Oak → naming → overworld, in loop senza crash",
          "Milestone 3 — overworld vivo: rendering mappa, movimento, box di dialogo",
          "Audio: driver PSG SN76489 che canta — SFX, versi e musica cross-bank, envelope software",
          "Homecall rientranti: la scelta dello starter e la battaglia col rivale si risolvono finalmente",
          "Passata di correttezza overworld: tilemap al load mappa, seam dello scroll a delta, layer window emulato, flip sprite al volo",
          "Milestone 4 — un loop di gioco completo e giocabile: battaglie casuali, livellaggio, negozi, il parcel di Oak, il Pokédex",
          "Rifiniture grafiche (flip nei menu, un glifo nel dialogo del lab, un hang della musica), limiti hardware GG e validazione a gioco intero — la lunga salita",
        ],
      },
      "wsc-bringup": {
  ...
  ja: {
    ...
    platforms: {
      ...
      gamegear: { blurb: "セガのGame Boyへのカラーの回答：手のひらの中の、ほぼMaster System — 同じZ80、同じVDPの血統 — 任天堂の緑のモノクロには手が届かない、バックライト付きのカラー画面の奥に。代償は電池ぶたに記されていた：単三電池六本を数時間で飲み干す。明るく、喉が渇き、そしてMaster Systemのよく地図化された自作ソフトの道に恵まれている。" },
    ...
    projects: {
      ...
      "gg-pokered": {
        ...
        summary: "Game Boyを象徴づけたRPGを、セガのカラー携帯機へ運ぶ — 書き直すのではなく、オープンソースの逆アセンブルpokeredを、ゼロから作ったPythonのトランスパイラ（完全なRGBDSフロントエンド）でWLA-DXのZ80へトランスパイルして。その周りには、二つのマシンの間のあらゆるハードウェアの隔たりを埋める手書きのHALがある：メモリマップされたVRAMではなくポート経由で駆動するVDP、Game BoyのMBCの代わりを務めるセガのマッパー、セガのSATへ作り直したシャドウOAM、再配置したHRAM、そして一つずつ手なずけた微妙な罠 — 割り込みを再有効化してはならないreti、swapの正確なフラグ、位置を変えるSRAMの窓。そして壁は崩れた — 崩れ続けた。タイトル前の白画面は、積み重なった三つの別々のバグだと判明した。それを越えるとタイトル画面が現れ、オープニングの流れがすべて動く：メインメニュー、New Game、オーキド博士の話、名前入力、そしてオーバーワールドへ — クラッシュせずにループする。マップが描画され、プレイヤーが歩き、博士のダイアログボックスが開く。Game Gearは声も得た：手書きのPSG SN76489ドライバが、効果音・モンスターの鳴き声・ダイアログのビープを、ソフトウェアエンベロープで一対一に鳴らし、音楽のノートストリームは — バンク0のヘルパー経由でクロスバンク読み出しされ — タイトルの曲を高らかに奏でる。ここへ至るには、エミュレータ内でのライブウォッチポイント（DAP経由のEmulicious）による長い狩りが要った：homecallのトランポリンを消し去るWRAMのワイプ、マップバッファの中に住んでいたスタック、本物のNMOSの ld a,i 割り込みクセ、非アトミックなVDPラッチ、そして — 最も深いもの — 圧縮homecallを再入可能にすること。それによりスターター選択とライバル戦がついに解決し、ゲームは先へ進む。そしてそれはゲームになった。マップ読み込み時のタイルマップ、スクロールの継ぎ目、エミュレートしたウィンドウ層、その場でのスプライトミラーリングをすべて正したことで、オープニングのループ全体が端から端まで遊べるようになった：正しいデータで戦う野生の戦闘、レベリング、ビリジアンへ向かう101番道路の登り、マートで買うオーキドの小包、そしてメニューから受け取り機能するポケモン図鑑 — 歩くだけのデモが、いくつかの見た目のグリッチを除いて、本物のゲームになった。この先にはまだ：最後の見た目の仕上げ（メニューでのスプライトのフリップ、研究所の一つの台詞に取り憑くウムラウト、ところどころの音楽の小さなハング）、Game Gear固有のハードウェア制限（Game Boyの十に対しスキャンラインあたり八スプライト）を最終パスへ先送り、そしてゲーム全体にわたる検証。オープンなpokered逆アセンブルの上に築いた技術的な習作：ドキュメントのみ、配布物はなし、ゲームは持参のこと。",
        milestones: [
          ...
          "全50バンクがトランスパイル・アセンブル・リンクされ、1 MBの.ggへきれいに；Game Freakのイントロまで起動",
          "マイルストーン1 — タイトル画面：白画面の壁（積み重なった三つのバグ）を突破",
          "マイルストーン2 — 完全な流れ：New Game → オーキド → 名前入力 → オーバーワールド、クラッシュせずループ",
          "マイルストーン3 — 生きたオーバーワールド：マップ描画、移動、ダイアログボックス",
          "オーディオ：歌うPSG SN76489ドライバ — SFX、鳴き声、クロスバンク音楽、ソフトウェアエンベロープ",
          "再入可能なhomecall：スターター選択とライバル戦がついに解決",
          "オーバーワールドの正しさの一巡：マップ読み込み時のタイルマップ、デルタ・スクロールの継ぎ目、エミュレートしたウィンドウ層、その場でのスプライトフリップ",
          "マイルストーン4 — 完全に遊べるゲームループ：野生の戦闘、レベリング、店、オーキドの小包、ポケモン図鑑",
          "見た目の仕上げ（メニューのフリップ、研究所の台詞の一字、音楽のハング）、GGのハードウェア制限、ゲーム全体の検証 — 長い登り",
        ],
      },
      "wsc-bringup": {
